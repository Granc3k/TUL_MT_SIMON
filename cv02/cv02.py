...
def read_file(file_str: str):
    ...
    with open(file_str, "rb") as f:
        ...
        samples_per_channel = wav_data.get("data_size") // (
            wav_data.get("channels") * (bits_per_sample // 8)
        )
        SIG = np.zeros((samples_per_channel, wav_data.get("channels")))
        try:
            for i in range(samples_per_channel):
                for ch in range(wav_data.get("channels")):
                    sample = struct.unpack(sample_format, f.read(bits_per_sample // 8))[
                        0
                    ]
                    SIG[i, ch] = sample
        except struct.error:
    ...
    if wav_data.get("channels") > 1:
        fig, axs = plt.subplots(
            wav_data.get("channels"), 1, figsize=(20, 5 * wav_data.get("channels"))
        )
    else:
